...
<nav style="z-index:50;position:fixed;width:100%;overflow:hidden;top:0px"
    ...
    <a class="navbar-brand" style="font-size: 25px; color: white;cursor:pointer;" onclick="window.location.reload()">VSM
        Builds </a>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-default"
                                            onclick="copyContents('<?php echo $dir; ?>','<?php echo $develop ?>')" ;>
                                        <span class="glyphicon glyphicon-cloud-upload"></span> Development
                                    </button>
                                </td>
                                <td>
                                    <button type="submit" class="btn btn-default"
                                            onclick="copyContents('<?php echo $dir; ?>','<?php echo $stage ?>')" ;>
                                ...
                                <td>
                                    <button type="button" class="btn btn-default"
                                            onclick="copyContents('<?php echo $dir; ?>','<?php echo $release ?>')" ;>
                ...
                for (var i = 0; i < data.length - 1; i++) {
                    ...
                    if (data[i].name !== '.' && !(data[i].name === '..' && (base === dirpath || base === dirpath2))) {
                        ...
                        if (((data[i].name).toString()).includes('--')) {
                            ...
                            dynamicHtml += "</td><td style='padding-left:500px'>  <button type=\"button\" class=\"btn btn-default\" onclick=\"copyContents('/opt/builds/"+data[i].name+"','" + develop + "')\"; >\n" +
                                "      <span class=\"glyphicon glyphicon-cloud-upload\"></span> Development\n" +
                                "    </button></td>\n" +
                                "<td style='padding-left:30px'>  <button type=\"submit\" class=\"btn btn-default\" onclick=\"copyContents('/opt/builds/"+data[i].name+"','" + stage + "')\";>\n" +
                                "      <span class=\"glyphicon glyphicon-cloud-upload\"></span> Stage\n" +
                                "    </button></td>\n" +
                                "\n" +
                                "        <td style='padding-left:30px'>  <button type=\"button\" class=\"btn btn-default\" onclick=\"copyContents('/opt/builds/"+data[i].name+"','" + release + "')\";>\n" +
                                "      <span class=\"glyphicon glyphicon-cloud-upload\"></span> Release\n" +
                                "    </button></td>\n"
